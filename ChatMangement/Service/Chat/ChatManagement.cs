...
#pragma warning disable SKEXP0050
    public class ChatManagement : IChatManagement
    {
        ...
        public async Task<ChatOutput> ChatHandling(ChatInput value)
        {
            ...
            {
                if (chat.role == "user") 
                    history.AddUserMessage(chat.content);
            ...
            Message msMessage = new (){ role = final.Role.Label, content = final.Content! };
            var TotalToken = final.Metadata?["Usage"] as OpenAI.Chat.ChatTokenUsage;
            ChatOutput output = new ChatOutput()
            {
                OutputMessage = msMessage,TotalToken =TotalToken!.TotalTokenCount
            };
            return output;
        }
